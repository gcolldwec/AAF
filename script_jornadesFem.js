...
const selectJornadaFem = document.getElementById("selectJornadaFem");
const jornadaNombreFem = document.getElementById("jornadaNombre");
...
function mostrarJornadesFem(j) {
    jornadaFem.innerHTML = '';
    var settings = {
    ...
    $.ajax(settings).done(function (response) {
        ...
        results.forEach(function(element) {
            jornadaNombreFem.innerHTML = "<h2 class='text-center mt-3 mb-3'>Jornada " + j + "</h2>"
            ...
            if(j == jornada) {
                ...
                jornadaFem.innerHTML += partit_jornada;
                jornadaFem.innerHTML += '<hr>';
...
function mostrarTotesJornades() {
    ...
    $.ajax(settings).done(function (response) {
        ...
        results.forEach(function(element) {
            let round = element.league.round;
            let jornada = round.substring(16);
            jornadaNombreFem.innerHTML = "<h2 class='text-center mt-3 mb-3'>Jornada 1</h2>"
            ...
            if(j == jornada) {
                ...
                j++;
                jornadaFem.innerHTML += '<br><br>';
                jornadaFem.innerHTML += "<h2 class='text-center mt-3 mb-3'>Jornada " + jornada + "</h2>";
                jornadaFem.innerHTML += '<br>';
                let home = element.teams.home.name;
                let escutHome = element.teams.home.logo;
                let away = element.teams.away.name;
                let escutAway = element.teams.away.logo;
...
//funció que crea un objecte amb dos paràmetres, insereix l'objecte dins un array  i s'insereix l'array al local storage
    ...
    if(localStorage.length > 0 ) {
        ...
        for(let i = 0; i < localStorage.length; i++){
            ...
            if(partitJornada[0].divisio == 'femeni') {
                ...
                var partit_jornada = "<div class='row mt-2 d-flex justify-content-center'><div class='col-12'><div class='row partit " + index +"'><div class='col-1  text-center mx-3'><img id='escutDesc' src='./imatges/escuts_femeni/" + partitJornada[0].equipCasa + ".png' alt='escut'/></div><div class='col-9 text-center'><span id='equipCasa'>" + partitJornada[0].equipCasa + "</span id='guio'>-<span id='equipVisitant'>" + partitJornada[0].equipVisitant + "</span></div><div class='col-1 text-center ml-2 escut'><img id='escutDesc' src='./imatges/escuts_femeni/" + partitJornada[0].equipVisitant + ".png' alt='escut'/></div></div></div></div>"
                jornadaFem.innerHTML += partit_jornada;
                jornadaFem.innerHTML += '<hr>';
                index++;
            } 
...
selectJornadaFem.addEventListener("change", (e) => {
    jornadaTravessaValor = +e.target.value;
    if(jornadaTravessaValor == 0) {
        mostrarTotesJornades();
    } else {
        mostrarJornadesFem(jornadaTravessaValor);
    }
}, false);
//event on load que ens mostra per pantalla la jornada femenina generada al apartat travesses
